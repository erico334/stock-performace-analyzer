...
class ReportBuilder:
    ...
    def _write_metric_cards(self, ws, cards, start_row=3):
        ...
        ws.set_row(start_row+1, 26)
        ws.set_row(start_row+2, 8)
    ...
    def build_summary(self):
        ...
        for ri, row in bucket_df.iterrows():
            ...
            ws.write(r, 6, row['CapitalTied'],    self._cf(bg, {'num_format':'#,##0'}))
    ...
    def build_detail(self, sheet_name, title, subtitle, src_df):
        ...
        self._write_headers(ws, [
            '#', 'Item Name', 'Last Sale Date', 'Days Idle', 'Qty Sold',
            'Qty At Hand', 'Unit Cost', 'Capital Tied', 'Barcode', 'Status'
        ], 6)
        sdf = src_df.copy()
        ...
        for idx, row in sdf.iterrows():
            ...
            ws.write(r, 4,  int(days) if pd.notna(days) else 0,              self._cf(bg, {'num_format':'#,##0'}))
            ws.write(r, 5,  int(row['QTY SOLD']),                            self._cf(bg, {'num_format':'#,##0'}))
            ws.write(r, 6,  qty_h,                                            self._cf(bg, {'num_format':'#,##0', 'font_color':'#C00000' if qty_h < 0 else '#1F1F1F'}))
            ...
            ws.write(r, 9,  str(row.get('BARCODE_STR', '')),                  self._cf(bg))
    ...
    def build_monthly(self):
        ...
        self._write_headers(ws, ['Month', 'Active SKUs', 'Units Sold', 'Revenue', 'Avg Rev/SKU'], 3)
    ...
    def build_top_products(self):
        ...
        self._write_title(ws, 'TOP 50 PRODUCTS - BY REVENUE', 'Ranked by revenue. Red = low stock.', col_end=8)
        self._write_headers(ws, ['Rank', 'Item Name', 'Qty Sold', 'Qty At Hand', 'Unit Cost', 'Revenue', 'Last Sale', 'Status'], 2)
    ...
    def build_dead_stock(self):
        ...
        self._write_headers(ws, ['#', 'Item Name', 'Qty At Hand', 'Unit Cost', 'Capital Tied', 'Barcode', 'Recommendation'], 6, fmt_key='hdr_gray')
        for ri, row in dead.iterrows():
            ...
            ws.write(r, 6, str(row.get('BARCODE_STR', '')),                   self._cf(bg))
    ...
    def build_negative_stock(self):
        ...
        if len(neg) == 0:
            return
        ws = self.wb.add_worksheet('Negative Stock')
        ws.hide_gridlines(2)
        ws.set_column(0, 0, 3)
        ws.set_column(1, 1, 6)
        ws.set_column(2, 2, 44)
        ...
        self._write_title(ws, 'NEGATIVE INVENTORY - DATA INTEGRITY ALERT',
            str(len(neg)) + ' products show negative stock. Likely bulk-break/sachet tracking errors.', col_end=6)
        self._write_headers(ws, ['#', 'Item Name', 'Qty At Hand', 'Qty Sold', 'Unit Cost', 'Likely Cause'], 3, fmt_key='hdr_red')
    ...
    def build(self):
        ...
        self.build_detail('Slow Moving (31-90 Days)', 'SLOW MOVING - Last sale 31 to 90 days ago',
            str(len(slow)) + ' products | ' + str((slow['QTY AT HAND'] > 0).sum()) + ' with stock', slow)
        ...
        self.build_detail('Dormant (91-180 Days)', 'DORMANT - Last sale 91 to 180 days ago',
            str(len(dorm)) + ' products | ' + str((dorm['QTY AT HAND'] > 0).sum()) + ' with stock', dorm)
        nd = df[df['DAYS_SINCE_SALE'] >= 181].copy()
        self.build_detail('Near Dead (181+ Days)', 'NEAR DEAD - Last sale 181+ days ago',
            str(len(nd)) + ' products | ' + str((nd['QTY AT HAND'] > 0).sum()) + ' still have stock', nd)
        ...
        self.build_detail('All Stock - By Idle Days', 'ALL IN-STOCK PRODUCTS - Ranked oldest last-sale first',
            str(len(all_stock)) + ' products with QTY AT HAND > 0', all_stock)
        ...
        self.build_detail('All Sold - Chronological', 'ALL SOLD PRODUCTS - Full historical register oldest first',
            str(len(sold)) + ' products that have ever recorded a sale', sold)
        self.wb.close()
        self.buf.seek(0)
