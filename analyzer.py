...
def get_summary_metrics(df):
    ...
    top10      = (df.nlargest(10,"REVENUE")["REVENUE"].sum() / total_rev * 100) if total_rev > 0 else 0
    return {
        "total_skus":      total,
        "active":          int(active),
...
def get_top_products(df, n=20, by="REVENUE"):
    cols = ["ITEM NAME","QTY SOLD","QTY AT HAND","UNIT_COST","REVENUE",
            "LAST_SALE_STR","STATUS","DAYS_SINCE_SALE"]
...
def get_bucket_summary(df):
    ...
    rows.append({
        ...
        "CapitalTied":    ns_ws["CAPITAL_TIED"].sum(),
        "Avg Days Idle":  None,
        "Risk Level":     "Critical",
...
def get_slow_moving(df):
    ...
    return sub.sort_values(["DAYS_SINCE_SALE","CAPITAL_TIED"],
...
def get_dead_stock(df):
    ...
    sub["RECOMMENDATION"] = sub["CAPITAL_TIED"].apply(
        lambda x: "Liquidate urgently" if x > 50000
        else "Promote or bundle" if x > 10000
        else "Clear or de-list"
    )
...
def get_all_stock_by_idle(df):
    ...
    return sub.sort_values(["_sort","CAPITAL_TIED"],
                           ascending=[False,False]).drop(columns=["_sort"]).reset_index(drop=True)
